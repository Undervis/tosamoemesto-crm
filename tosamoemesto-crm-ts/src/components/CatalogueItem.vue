...
const styleMap = ['max-width: 12rem; max-height: 12rem', 'max-width: 6rem; max-height: 6rem']
...
function deleteItem() {
  if (confirm('Вы действительно хотите удалить запись?')) {
    axios
      .delete(
        `${gProps.getApiUrl()}${router.currentRoute.value.meta.apiEndPoint}/${props.item.id}/`,
      )
      .then(() => {
  ...
  <div class="card shadow-sm">
    <div class="row">
      <div class="col-auto pe-0">
        <img
          class="rounded-start img-fluid border-end"
          style="object-fit: cover; height: 100%; object-position: center; aspect-ratio: 1"
          :style="type === 'food' ? styleMap[0] : styleMap[1]"
      ...
      <div class="col d-flex flex-column ps-0">
        <div class="card-header p-2">
          <div class="hstack gap-2 align-items-center">
            ...
            <span v-if="item.sizes_and_prices" class="text-muted" title="Минмальная цена"
              >от {{ Math.ceil(item.sizes_and_prices[0].price) }} ₽</span
            >
        ...
        <div v-if="item.sizes_and_prices" class="card-footer p-2 hstack gap-1">
          <span
            v-if="item.active"
            class="alert alert-success p-0 px-2 text-success m-0 small rounded-pill"
            title="Статус доступности"
            >Активно</span
          >
          <span
            v-else
            class="alert alert-danger p-0 px-2 text-danger m-0 small rounded-pill"
            title="Статус доступности"
            >Не активно</span
          >
          <span
            class="alert alert-primary p-0 px-2 text-primary m-0 small rounded-pill"
            title="Категория блюда"
            >{{ item.category.title }}</span
          >
          <span
            v-for="size in item.sizes_and_prices"
            class="alert alert-info p-0 px-2 text-info m-0 small rounded-pill"
            title="Досутпные размеры"
          >
