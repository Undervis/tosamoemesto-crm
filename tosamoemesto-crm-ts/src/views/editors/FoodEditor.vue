...
onBeforeMount(() => {
  ...
  if (Number(current_id) > 0) {
    loading.value = true
    axios
      .get(`${gProps.getApiUrl()}${router.currentRoute.value.meta.apiEndPoint}${current_id}`)
      .then((response) => {
        ...
        food_item.value.accepted_additions = response.data.accepted_additions.map(
          (item: Addition) => item.id,
        )
  ...
  save() {
    if (form.value.checkValidity()) {
      ...
      if (Number(current_id) > 0) {
        ...
        axios
          .patch(
            `${gProps.getApiUrl()}${router.currentRoute.value.meta.apiEndPoint}${current_id}/`,
            data_to_send,
          )
          .then((response) => {
        ...
        axios
          .post(`${gProps.getApiUrl()}${router.currentRoute.value.meta.apiEndPoint}`, data_to_send)
          .then((response) => {
            useToast().success('Сохранено')
          })
          .catch((error) => {
...
<template>
  <section class="container-fluid overflow-y-scroll">
    ...
    <div class="hstack p-2">
      ...
      <button type="submit" @click="food_item.save()" class="btn btn-outline-success ms-auto">
    ...
    <form ref="form" class="p-2 gap-2 d-flex flex-column" @submit.prevent>
      <div class="hstack gap-2 flex-wrap">
        ...
        <div class="form-floating w-100">
          <input
            class="form-control"
            v-model="food_item.title"
            type="text"
            id="foodTitle"
            placeholder="Паста балоньезе"
            required
          />
      ...
      <div class="form-control p-0">
        <QuillEditor
          theme="snow"
          v-model:content="food_item.description"
          content-type="html"
          ...
          <div class="form-floating">
            ...
            >
              ...
              <option
                ...
                :selected="food_item.category === item.id"
                :key="item.id"
                :value="item.id"
              >
          ...
          <div class="input-group">
            ...
            <div class="form-floating">
              <input
                ...
                min="0"
                type="number"
                v-model="food_item.cooking_time"
                class="form-control"
                placeholder="0"
              />
      ...
      <div class="row">
        <div class="col-auto">
          <ImageInput
            :imgUrl="food_item.image"
            @uploaded="(image: string) => (food_item.image = image)"
          />
        </div>
        <div class="col">
          <div class="vstack gap-1">
              ...
              <li v-for="size in food_item.sizes_and_prices">
                <div class="d-flex flex-wrap input-group">
                  ...
                  <div class="input-group-text">
                    <button
                      type="button"
                      @click="food_item.removeSize(size)"
                      :disabled="food_item.sizes_and_prices.length === 1"
            ...
            <button
              type="button"
              @click="food_item.addSize()"
              :disabled="food_item.sizes_and_prices.length >= 7"
        ...
        <div class="col">
          <div class="vstack gap-1">
            ...
            <items-selector
              @checked="(items: Addition) => checkAdditions(JSON.stringify(items))"
              v-if="typeof additions === typeof ref([])"
              :data="additions"
            />
          </div>
        </div>
